...
class Biblioteca:
    ...
    # === MÉTODO ÚNICO DE PESQUISA ===
    def pesquisar(self, tipo, termo="", status=None):
        t = (termo or "").strip().lower()

        if tipo == "item":
            colecao = self.catalogo.values()
            chave = lambda i: f"{i.codigo} {i.titulo} {i.tipo}"
            filtro_extra = None

        elif tipo == "usuario":
            colecao = self.usuarios.values()
            chave = lambda u: f"{u.id_usuario} {u.nome}"
            filtro_extra = None

        elif tipo == "emprestimo":
            colecao = self.__emprestimos
            chave = lambda e: f"{e.item.codigo} {e.item.titulo} {e.usuario.id_usuario} {e.usuario.nome}"
            if status == "aberto":
                filtro_extra = lambda e: e.em_aberto
            elif status == "devolvido":
                filtro_extra = lambda e: not e.em_aberto
            else:
                filtro_extra = None

        else:
            raise ValueError("Tipo inválido. Use: item | usuario | emprestimo")

        def combina(obj):
            if t and t not in chave(obj).lower():
                return False
            if filtro_extra and not filtro_extra(obj):
                return False
            return True

        return [obj for obj in colecao if combina(obj)]
        ...
        if escolha == "0":
            ...
            while True:
                ...
                if op == "1":
                    ...
                    achados = bib.pesquisar("item", termo)
                    if not achados:
                    ...
                    termo = input("Termo (se vazio lista todos): ").strip()
                    achados = bib.pesquisar("usuario", termo)
                    if not achados:
                    ...
                    status = input("Status [enter=Todos | aberto | devolvido]: ").lower() or None
                    achados = bib.pesquisar("emprestimo", termo, status)
                    if not achados:
